    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
            ...
            if (isset($packages[$type])) {
                ...
                foreach ($packages[$type] as $package) {
                    ...
                    if (is_object($package)) {
                        ...
                        $packageViewData['releaseDate'] = $package->getReleaseDate();
                        $packageViewData['versionReleaseDate'] = $package->getReleaseDate()->format('d-M-Y');
                        $packageViewData['latestReleaseDate'] = $latestPackages[$package->getName()]->getReleaseDate()->format('d-M-Y');
                        
                        $packageViewData['url'] = $package->getDistUrl();
                        $packageViewData['homepage'] = $package->getSourceUrl();
                        
                        $latestReleaseDate = $packageViewData['latestReleaseDate'];
                        $versionReleaseDate = $packageViewData['versionReleaseDate'];
                ...
                $viewMetaData[$type] = array(
                    ...
                    'versionReleaseDateLength' => strlen($versionReleaseDate),
                    'latestReleaseDateLength' => strlen($latestReleaseDate),
                );
        ...
        if ('json' === $format) {
            ...
            foreach ($viewData as $type => $packages) {
                ...
                $latestReleaseDateLength = $viewMetaData[$type]['latestReleaseDateLength'];
                $versionReleaseDateLength = $viewMetaData[$type]['versionReleaseDateLength'];
                
                $writeVersion = $nameLength + $versionLength + 3 <= $width;
                $writeVersionReleaseDate = $nameLength + $versionLength + $versionReleaseDateLength + 3 <= $width;
                $writeLatest = $nameLength + $versionLength + $versionReleaseDateLength + $latestLength + 3 <= $width;
                $writeLatestReleaseDate = $nameLength + $versionLength + $versionReleaseDateLength + $latestLength + $latestReleaseDateLength + 3 <= $width;
...
// HERE LATEST VERSION OUTPUT IS GENERATED
                foreach ($packages as $package) { // RELEASE DATE
...
//                    if (isset($package['releaseDate'])) {
//                        $io->write($package['releaseDate']->format('d M Y'));
//                    }
                    
                    
                    if (isset($package['versionReleaseDate']) && $writeVersionReleaseDate) {
                        $versionReleaseDate = $package['versionReleaseDate'];
                        $io->write(' |' . str_pad($versionReleaseDate, $versionReleaseDateLength, ' ').'|', false);
                    }
                    ...
                    if (isset($package['latestReleaseDate']) && $writeLatestReleaseDate) {
...
//                        $io->write($latestPackageReleaseDate);
//                    }
